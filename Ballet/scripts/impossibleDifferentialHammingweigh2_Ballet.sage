...
def save(filename,mod,string):
    try:
        ...
        error_data = f"Exception occurred while saving results on {filename} in mod {mod} ({sys.argv}): {str(e)}\n" + "-" * 60 + f" end={datetime.datetime.now()}\n\n"
...
def searchImpossibleDifferentials(pt_combinations,ct_combinations,block_size,key_size,num_rounds,filename):
    ...
    for pt_bits in pt_combinations:
        for ct_bits in ct_combinations:
            ...
            try:
                ...
                if result is not None and result.get("status") == "UNSATISFIABLE":
                    msg = '\t\t{'+f'"pt_bits": {list(pt_bits)}, "ct_bit": {list(ct_bits)}, "time_seconds": {round(elapsed_time, 4)}'+'},\n'
                    save(filename,"a",msg)

            except Exception as e:
                error_message =  f"args={sys.argv}\nException occurred during SAT ({num_rounds}r {block_size}b {key_size}k {pt_bits}pt_bits {ct_bit}ct_bit): {repr(e)}\n\n" + f"Traceback:\n{traceback.format_exc()}" + "-" * 60 + f" time={datetime.datetime.now()}\n\n"
...
def parallel_resolution(filename,num_rounds,block_size, key_size,c):
    ...
    with ProcessPoolExecutor(max_workers=c) as executor:
        for i in range(c):
            if i != c-1:
                subset_pt_combinations = pt_combinations[pt_comb_partial_len*i:pt_comb_partial_len*(i+1)]
            else:
                subset_pt_combinations = pt_combinations[pt_comb_partial_len*i:]
            new_filename = filename + f"_{i}.tmp"
            save(new_filename,"w","")

            executor.submit(
                searchImpossibleDifferentials,
                subset_pt_combinations,
                ct_combinations,
                block_size,
                key_size,
                num_rounds,
...
for block_size, key_size in configurations:
    for num_rounds in range(1, 2):
        filename = f"impossibile_differential_hammingWeight2__ballet_{num_rounds}round_{block_size}block_{key_size}key__{solver}solver_{c}thread.json"
