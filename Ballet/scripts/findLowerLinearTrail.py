...
import traceback
import datetime
# note: you must manually add the new method (./newLibraryMethods/find_one_lower_linear_trail.py) inside the library to run this file
...
ballet_chipers = [(BalletBlockCipher(number_of_rounds=i), i) for i in [16]]
...
for ciphers, name in zip(listOfListOfCipher,["ballet128"]):
    for cipher, rounds in ciphers:
        ...
        sat = SatXorLinearModel(cipher)

        #call with timeout the new function
        beg1 = time.time()
        try: 
            result, test_message = sat.find_one_lower_weight_xor_linear_trail_having_max_waist_time_bounded(minutes=minutes, start=68, solver_name=solver)
        ...
        try: 
            ...
            with open(f"risFindLowerLinear_{minutes}min_{core}core.csv","a") as f:
                f.write(s)
            with open(f"search_and_wasted_times_linear_{minutes}min_{core}core.csv", "a") as f:
                f.write(f"{cipher.family_name}, {rounds}, {round(end1-beg1,2)}, " + test_message)
            with open(f"find_one_lower_weight_xor_linear_trail__{name}_{rounds}rounds_{minutes}minutes_{core}core__{weight_new}weight__{solver}solver.txt","w") as g:
                g.write(str(result))
        except Exception as e:
