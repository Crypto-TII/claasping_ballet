# this method must be added in the sat_xor_linear_model.py library of CLAASP

    def find_one_lower_weight_xor_linear_trail_having_max_waist_time_bounded(
        ...
        def save_tmp_trail(actual_solution):
            if actual_solution is None:
                return
            file_path = f'find_one_lower_weight_xor_linear_trail__{actual_solution["cipher"]}_weight{int(actual_solution["total_weight"])}.tmp'
            try:
        ...
        def find_one_lower_weight_trail(max_weight):
            ...
            self.build_xor_linear_trail_model(weight=max_weight, fixed_variables=fixed_values)
        ...
        def find_lower_weight_xor_linear_trail_within_timeout(start, timeout):         
        ...
        a_trail = self.find_one_xor_linear_trail() # assume it's time << minutes*60
        ...
        solution= find_lower_weight_xor_linear_trail_within_timeout(
        ...
        while solution != None and solution["total_weight"] is not None:
            ...
            if autosave_immediate:
                ...
                actual_solution["test_name"] = "find_one_lower_weight_xor_linear_trail"
            ...
            t = minutes*60 #+ 1.5*max(solving_times)
            solution = find_lower_weight_xor_linear_trail_within_timeout(
        ...
        # optional message understand the behavior and if it can be optimized (print the necessaty time, the wasted time and the percentage of time not wasted)
