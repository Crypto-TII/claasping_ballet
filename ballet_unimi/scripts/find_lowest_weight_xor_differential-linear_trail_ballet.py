...
for number_of_rounds in range(6,12):
    bottom_round = number_of_rounds//2
    ...
    for block_bit, key_bit in [(128, 128), (128, 256), (256, 256)]:
        ...
        for round_number in range(2, bottom_round):
    ...
    fixed_values=[key, plaintext, ciphertext_difference],            solver_name=solver,   
            ...
            with open(
                f"find_lowest_weight_xor_differential-linear_trail__ballet_{block_bit}block_{key_bit}key_{number_of_rounds}rounds__{solver}solver.txt",
                "a"
            ) as f:
